...
if (-not [string]::IsNullOrWhiteSpace($SolarReturnRunDir) -and (Test-Path $SolarReturnRunDir)) {
  ...
  # SR nodal axis (where the year pulls toward / releases) — position + SR house.
  foreach ($pt in $srPts) { if ($pt.point -in @("North Node","South Node")) { $pl=[double]($pt.longitude -replace ',','.'); $hn=""; for ($h=0;$h -lt $srH.Count;$h++){ $c1=[double]($srH[$h].longitude -replace ',','.'); $c2=[double]($srH[($h+1)%$srH.Count].longitude -replace ',','.'); $inb= if($c1 -lt $c2){($pl -ge $c1 -and $pl -lt $c2)}else{($pl -ge $c1 -or $pl -lt $c2)}; if($inb){$hn=$srH[$h].house} }; Add-Row "соляр" @("sr","point",$pt.point) "точка соляра: $($pt.point)" "$($pt.sign) $([math]::Round([double]($pt.degree -replace ',','.'),1)) · дом соляра $hn" "" } }
