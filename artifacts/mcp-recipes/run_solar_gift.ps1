...
# ── 3. TRANSITS over the solar year. Scan a SYMMETRIC ±3-mo margin around the 12-month year
#     (SR−3mo → SR+15mo) so zone classification (#84) can sample BOTH tail (slow carrier perfecting
#     just BEFORE the year, still in orb at open) and horizon (perfecting just AFTER next SR). The scan
#     window is DECOUPLED from the year anchor: -SolarYearStartUtc stays = SR-instant, and the recipe
#     drops any window that closes before SR (last year's themes). Without the backward margin, tail is
#     structurally unsampleable and the zone enum silently collapses to core/horizon (NKS astrolab #84).
Stage "transit windows"
$rangeStart = $srYearStart.AddMonths(-3).ToString("yyyy-MM-ddT00:00:00Z")
$rangeEnd   = $srYearStart.AddMonths(15).ToString("yyyy-MM-ddT00:00:00Z")
Recipe "run_transits_to_natal.ps1" @("-CaseId",$chartId,"-Latitude",$Latitude,"-Longitude",$Longitude,"-BirthDateTimeUtc",$utc,"-RangeStartUtc",$rangeStart,"-RangeEndUtc",$rangeEnd,"-StepDays",7,"-Orb",1,"-SolarYearStartUtc",$srInstant,"-OutputBase",$runs)
...
Колонка `зона` у несущего окна (если есть) — это ФОРМА темы в году, не её важность (важность ставишь ты
по корроборации). Три значения: **ядро** — пик приходится В год (тема разворачивается этой год, читается
прямо); **горизонт** — пик за следующим ДР, но подход уже идёт (тема ВЫЗРЕВАЕТ — нота «нарастает к концу
года и раскроется дальше», не кульминация сейчас); **хвост** — пик был ДО года, тема ещё в орбе на старте
(ЗАВЕРШАЕТСЯ — нота «доигрывается, отпускает», не свежий импульс). Зона задаёт ТОН и место окна в дуге
года, а не вес: горизонт/хвост на заряженной точке могут быть весомее тихого ядра. Год дышит — у него
есть незавершённый вход (хвост) и приоткрытый выход (горизонт); не подавай его как ровную коробку 12 мес.
...
не заливай водой. Колонки `zone_core/zone_horizon/zone_tail` + `posture` — это ВРЕМЕННАЯ поза датирован­
ного (транзитного) заряда сферы, ось-близнец зон окна: **ядро** — сфера активна в этом году; **горизонт**
— вызревает (раскроется к концу года и дальше); **хвост** — завершается, доигрывает прошлогоднее. Это
ОРИЕНТИР для тона, не важность: сфера с одним хвостом — про «отпускание», с горизонтом — про «зреет»; на
тихой сфере горизонт спасает от пустоты («ядра нет, но вызревает X»). posture считается только по
транзитам — сфера может быть живой стоячим наталом при пустой позе (нет датированных событий — но тема
есть). Один фактор
