...
1. `twin.md` — твои рабочие заметки: фактура из данных + твоя логика, чтобы держаться карты.
   Структурируй как удобно тебе. В КОНЦЕ — короткий список факторов, которые ты осознанно НЕ понёс в
   отчёт, по одной строке «почему» (фон/дубль темы/слабый орб). Это твоя само-проверка, что фактор
   отброшен сознательно, а не потерян молча.
2. **`../packs/coverage_dispositions.csv` — ЗАПОЛНИ ОБЯЗАТЕЛЬНО.** Рядом с этим пакетом лежит реестр
   обхода: в нём строка на КАЖДЫЙ из ~200 факторов карты, и сейчас они пустые-болванки. Для каждой
   строки проставь `salience` (несущий / тихий / пропущено-осознанно) и `basis` (короткое «почему»,
   ссылка на factor_id-основания через `;`). Пустая строка = HOLE = «лысый соляр». ⚠️ ГЕЙТ С ЗУБАМИ:
   деливерабл НЕ соберётся, пока хоть одна строка без salience — это не пожелание, это блок.
3. **`../packs/year_roles.csv` — РАЗРЕШИ РОЛЬ.** Для ключевых планет года (хозяин года, управитель карты)
   впиши `role` — ОДНУ роль на планету («за/против/нейтрально + чем именно занят этот год»). Эта роль
   ЕДИНА во всех сферах из колонки `spheres`: не «Венера за меня» в любви и «против» в деньгах. Пустой
   `role` тоже блокирует деливерабл (тот же гейт).
4. `prose.md` — тёплая русская проза ПО РАЗДЕЛАМ готового отчёта (структура ниже): показывай, не
...
## Сферы жизни — ОТДЕЛЬНАЯ ось, не пересказ окон
Окна = ось ВРЕМЕНИ (когда что). Сферы = ось ОБЛАСТИ ЖИЗНИ (какова эта область в год как целое). Это
РАЗНЫЕ вопросы — сферу пиши ОТ НАТАЛА, не из окон: (1) как эта область устроена от рождения (дома сферы
+ их управители + жильцы + аспекты); (2) что соляр говорит про неё (SR-планеты в натальных домах сферы,
sr→натал, склонения); (3) сдвиг (прогрессии/дирекции на управителей сферы); (4) итог года. Даты —
ПРИМЕЧАНИЕ, не скелет («лучший момент — осень»), не пересказывай главу-окна.
`sphere_summary.csv` — какие сферы заряжены, а какие ТИХИЕ: тихую назови честно («спокойный год по X»),
НЕ заливай водой. `sphere_routing.csv` — какие факторы относятся к каждой сфере (кандидаты; СМЫСЛ и вес
решаешь ты). Один фактор может быть в нескольких сферах (Венера → и любовь, и деньги) — это норма, но
роль планеты на год держи ЕДИНОЙ во всех сферах (не «за меня» в одной и «против» в другой). Тон фактора,
если он и в окне, и в сфере — ОДИН (валентность из диспозиции, не пересчитывай).
...
if ([string]::IsNullOrWhiteSpace($ModelAdapter)) {
  ...
  Write-Host "  adapter done — twin/prose produced + self-check passed. (template fill + PDF: next stage, TODO)"
}
