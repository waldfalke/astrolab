...
2. **`../packs/coverage_dispositions.csv` — ЗАПОЛНИ ОБЯЗАТЕЛЬНО.** Рядом с этим пакетом лежит реестр
   ...
   ссылка на factor_id-основания через `;`). Пустая строка = HOLE = «лысый соляр» = блок на сборку PDF.
   Это и есть само-проверка полноты: пройти ВСЕ факторы и для каждого решить «несу / отбрасываю».
3. `prose.md` — тёплая русская проза ПО РАЗДЕЛАМ готового отчёта (структура ниже): показывай, не
...
foreach ($sf in @("sphere_summary.csv","sphere_routing.csv")) {
