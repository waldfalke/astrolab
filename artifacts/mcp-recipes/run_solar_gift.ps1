...
1. `twin.md` — твои рабочие заметки: фактура из данных + твоя логика, чтобы держаться карты.
   Структурируй как удобно тебе. В КОНЦЕ — короткий список факторов из `coverage_factors.csv`, которые
   ты осознанно НЕ понёс в отчёт, по одной строке «почему» (фон/дубль темы/слабый орб). Это не для
   клиента — это твоя само-проверка, что фактор отброшен сознательно, а не потерян молча. Заполни
   `coverage_dispositions.csv` (salience каждому): иначе видно дыры обхода («лысый соляр»).
