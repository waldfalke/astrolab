...
| A4 | **All 7 spheres charged=True.** `charged` set by presence of ANY routed factor, not weight — domain axis never says "тихий". | RESOLVED **not by a weighted threshold** — that would compute importance by formula, which is a ROLE-in-the-whole, not a factor PROPERTY (crosses the determinization line, NKS #85). Instead: charged stays a coarse raw signal ("has factors / none"); the model judges charged-vs-тихий from the factors + their orbs in sphere_routing, sharpened by a SECOND PASS (#86). BRIEF reworded: summary is raw material, not a verdict. |
